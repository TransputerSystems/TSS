...
import uk.co.transputersystems.transputer.assembler.Assembler;
...
public class SimulateFileTest {
     ...
     */
    public File testAssemblingFile(File file) throws Exception {
     ...
     * @return true if the compilation appears to have succeeded, otherwise false.
     */
    public File testAssemblingFile(File file, boolean verbose, AssemblerConfig config) throws Exception {
        ...
        File outputFile = new File(file.getPath().replace(".auto.", ".").replace(".s",".auto.o"));
    ...
    public File testCompilingFile(InputStream inputStream, String filePath, String fileName, boolean verbose) throws Exception {
        uk.co.transputersystems.occam.ErrorListener errorListener = new ErrorListener();
    ...
    public boolean testSimulatingFile(List<File> binaries) throws Exception {
        List<String> args = new ArrayList<>();
        args.add(0, "--binaries");
        args.add(binaries.stream().map(File::getPath).collect(Collectors.joining(";")));
        args.add("--print-workspace-mem");
        Simulator.run(args.toArray(new String[0]));
        return true;
    }
